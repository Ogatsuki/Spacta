...
### D-010: パッケージが利用者の木に書き込んでよいのは `.claude/` 配下だけ
決定日 2026-08-15（v0.12）/ 実測 `spacta-init --dry-run` の書き込み先 **2件、どちらも `.claude/` 配下**

`npm install spacta` は**何も書き込まない**（`postinstall` 等の消費者側ライフサイクルを1つも定義していない）。
書き込むのは `npx spacta-init` を人が明示的に叩いたときだけで、書いてよい先は `.claude/` 配下に限る。

**触ってよいもの（＝製品そのもの）:**
`.claude/skills/spacta/` と `.claude/hooks/spacta-verify-on-stop.mjs`。
Spacta は「AI が掟を守って書くための仕組み」であり、**この2つは目的の実装そのものである。**
配らなければ製品が成立しない。

**触ってはいけないもの:**
CI のワークフロー、git hook（pre-commit 等）、`package.json`、`tsconfig.json`、`src/**`、
lint 設定 —— **利用者のワークフローと成果物。** ここは目的の外側であり、
「ついでに整えてあげる」は親切ではなく越権である。`.claude/settings.json` は境界線上なので、
**既存ファイルは `--write-settings` を明示されない限り触らず**、渡されたときも
`.bak` を取ってからマージする（新規作成のときだけ無条件に書く）。

**勧めることは踏み込みではない。** 線は「助言か否か」ではなく **「書き出すか否か」**に引いてある。
`setup.md` が「`verify` を CI ゲートにせよ」と勧めるのは構わない —— それは Spacta の中心主張
（掟は強制されて初めて掟である）の説明であり、読むかどうかは相手が決める。
**駄目なのは、相手のワークフローを勝手に書き換えることである。** 助言は断れるが、
書き込まれたファイルは気づかれないまま残る。

**そのうえで、助言の口調は別途直した（2026-08-15）。** `setup.md` の
「検査を空にすることは**厳禁**」は、他人のリポジトリに対して言う言葉としては越権だった。
禁止ではなく**費用の非対称**を述べる形に書き換えてある ——
消した掟は毎回 `NOT verified in this project` に印字されるが、
空にした掟は `Guaranteed by this green` に印字され、**通った掟と区別がつかない。**
避けるべきは検査が無いことではなく、**走っていない検査を走ったと名乗る緑**である。

**なぜ明文化するか。** この線はこれまで **`init.mjs` の実装が事実上の答え**になっているだけで、
どこにも書かれていなかった。次に誰かが「CI の YAML も書き出せば親切だ」と思いついたとき、
止めるものが何も無い。**この文書の存在理由そのものの穴である。**
`.github/workflows/ci.yml` はこのリポジトリ自身のものであり、配布物に入っていない（`files` に無い）
—— それも「たまたま」ではなく、この決定の帰結として保つ。

**守る検査:** `tools/smoke-package.mjs`。
(1) `package.json` に消費者側ライフサイクル（`preinstall`/`install`/`postinstall`/`prepare`）が
1つも無いこと、(2) `spacta-init --dry-run` が書くと言う先が**全部 `.claude/` 配下**であること、
(3) `.github/` が同梱されていないこと。**(2) が本体である** —— 書き込み先が増えたら、
それが `.claude/` の外なら赤くなる。

**確かめ方:** 3つとも植えて落ちるのを見た。`init.mjs` の plan に
`.github/workflows/spacta.yml` を1件足す → **(2) が exit 1 で、その行を名指しした。**
...
| **利用者の木に書き込むのは `.claude/` 配下だけ** | **`smoke-package`** の3件（消費者側ライフサイクル無し / `init --dry-run` の書き込み先 / `.github` 非同梱） | ✅ 3件とも植えて落ちるのを見た（D-010） |
| **配布物のリンクが配布物の中で解決する** | **`smoke-package`** の同梱文書リンク走査 | ✅ 植えて落ちるのを見た（0.12） |
| 掟は10本のまま | **なし（無防備）** | — |
