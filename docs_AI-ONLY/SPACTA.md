...
*   **What remains in a shell is JSX wiring: state into props, callbacks into `dispatch`. Nothing else.** The mechanism a shell used to carry — holding state, minting `now` and ids, looping over Effects — belongs to the runtime and its React adapter (`spacta/runtime` and `spacta/react`). Do not write `useState` for feature state, `new Date()` or `crypto.randomUUID()` in a feature: call `useSpacta` and let it mint. Non-determinism still reaches Core as Action values (L3); what changed is only who mints it.
...
5.  Once implementation is complete, **run `npm run verify` — or `npx spacta-verify .`, which is the same file out of `node_modules` — yourself and fix all errors until green** (L6).
