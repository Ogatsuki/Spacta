# Membrain Alpha Evaluation Archive (Retired Insights)

This document preserves insights that were previously in `docs/membrain-alpha-evaluation.md` (Alpha Evaluation core) but have been removed from the current evaluation due to expired assumptions or changes in design decisions.
...
Conclusion: SAB is not adopted as a recommended technique in Membrain v4. It is recorded as a legacy technique but will not be included in current conventions or starter guides.
...
Furthermore, how to link specifications with implementation is not treated as a problem to be solved by this alpha evaluation or the Membrain framework. That is left to the user's operational judgment.

---

## Abandonment of Old Membrain "All-Definitionism" and Introduction of §2.5 (Judgment) [Established]

The old Membrain attempted to define everything in the conventions (e.g., where to place types, how to split shells, which logic to move to Core) to answer every question in design discussions. However, Next.js applications vary widely, and practice proved that locking forms down too rigidly failed to fit real applications.
...
The declaration at the beginning of the current `MEMBRAIN.md`—"As long as verify is green and the philosophy of §1 is followed, the details of the Form do not matter"—is a consequence of this abandonment.
...
"Abandonment of Old Membrain All-Definitionism" was moved from the historical prose at the beginning of old `Membrain/MEMBRAIN.md` §2.5.
The other three items were moved almost verbatim from the chapters of the same name in the old `docs/membrain-alpha-evaluation.md`.
Primary sources for context are `docs/history/dev/260628~/00_old-membrain-doc.md` (old v3 CLAUDE.md) and `01_membrain-bloat-ronko.md`.
