...
1. **Context Isolation**: Imagine trying to build something Amazon-sized as a Next.js app, with an AI agent doing the work. Even five years from now, when models can hold the entire codebase in context, that alone won't save you: reading everything isn't the same as guaranteeing that a change to checkout didn't quietly break inventory. Spacta's answer is physical isolation — every feature is boxed into its own directory (`core.ts` / `shell.tsx` / edge files) and cannot import another feature's internals. When you (or an AI) touch one feature, you only ever need that feature's own files — not the whole `app/` tree. In early evaluations, multiple independent AI agents implemented separate features in parallel against a frozen contract and integrated with zero collisions. This isn't only about how much an AI can read — reasoning through tangled, cross-cutting causality is a fundamentally harder task for AI than churning through repetitive, mechanical structure. Isolation keeps each unit of work mechanical and local, playing to that strength instead of fighting it.
...
* **Verbosity Isn't a Cost to AI the Way It Is to Humans**: Verbosity that would exhaust a human to write by hand — exhaustive switch cases, an explicit `Action`/`Effect` type for every branch — doesn't cost AI accuracy the same way it costs human patience. Repetitive, explicit structure tends to be exactly the kind of pattern AI reproduces most reliably; the failure mode to worry about is compressed, implicit logic, not verbose, mechanical logic. *(Whether this trade-off is worth the extra output tokens — and their cost — at scale is something we haven't rigorously measured yet; flagged here as an open question, not a settled claim.)*
